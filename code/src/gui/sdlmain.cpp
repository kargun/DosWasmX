...
char fps_text[250];
char fat_drive_mount[250];
char iso_drive_mount[250];
...
//          || (fullscreen && ((width != currWidth) || (height != currHeight)))
        ...
        sdl.window = SDL_CreateWindow(NULL,
...
# endif
    ...
    MAPPER_AddHandler(&PauseDOSBox, MK_pause, MMODHOST, "pause", "Pause emulation", NULL);
...
bool DOSBOX_parse_argv() {
    ...
    while (control->cmdline->GetOpt(optname)) {
        ...
        if (optname == "v" || optname == "ver" || optname == "version") {
            ...
            fprintf(stderr,"\nDOS Wasm X version %s %s, copyright 2023-%s The DOS Wasm X Team.\n",VERSION,SDL_STRING,COPYRIGHT_END_YEAR);
            fprintf(stderr,"DOS Wasm X project maintainer: nbarkhina\n\n");
            fprintf(stderr,"DOS Wasm X comes with ABSOLUTELY NO WARRANTY.  This is free software,\n");
...
int main_sdl(int argc, char* argv[]) {
    ...
    if (!strcasecmp(imestr, "auto")) {
        ...
        LOG(LOG_MISC,LOG_NORMAL)("Copyright 2023-%s The DOS Wasm X Team. Project maintainer: nbarkhina.",std::string(COPYRIGHT_END_YEAR).c_str());
...
fresh_boot:
        ...
        try {
            ...
            if (x == 0)
            {
                //instead of shutting down we want to send it back to the dos shell
                x = 9;
            ...
            if (x == 2) { /* booting a guest OS. "boot" has already done the work to load the image and setup CPU registers */
                ...
                //set the autoexec to automount the C and D drives
...
void set_autoexec_internal()
{
    ...
    sprintf(mountCommands, "%s%secho Type \"boot c:\" to boot into Windows\r\n%s", 
        fat_drive_mount, iso_drive_mount, neil_autoexec_additional);

    Section_line* section = static_cast<Section_line*>(control->GetSection("autoexec"));
    section->data = mountCommands;

    //clear additional commands because we only want to run them once
...
extern "C" {
    ...
    void neil_update_autoexec_additional(char* additional)
    {
        sprintf(neil_autoexec_additional, additional);
        set_autoexec_internal();
    }
    ...
    void neil_send_dos_commands(char* commands)
    {
